...
const Page = () => {
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ["listing-pricing", listingId],
    queryFn: async () => {
  ...
  useEffect(() => {
    if (!listingId || price === undefined) return;

    if (!debouncedSave.current) {
      debouncedSave.current = debounce(async (newPrice: string) => {
        if (!newPrice.trim() || newPrice === lastSavedPrice.current) return;
        try {
          setIsSaving(true);
    ...
    debouncedSave.current(price);

    return () => debouncedSave.current?.cancel();
  }, [price, listingId, current]);
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      if (price && price !== lastSavedPrice.current) {
        await api.patch(`/listings/${listingId}`, {
          price: Number(price),
          currentStep: current,
        });
        ...
        <div className="max-w-2xl mx-auto w-full h-full">
          <div className="w-full h-full my-auto max-h-[300px] flex flex-col justify-between">
            <div>
              ...
              <p className="text-base font-[550] text-black/60">
                Tip: ₵40. You’ll set a weekend price next.
              </p>
            </div>
              ...
              <div className="inline-block relative">
                ...
                {showButton && (
                  <div className="absolute bottom-0 -right-8">
                    <Button
                      variant="outline"
                      className="rounded-full p-0 aspect-square h-8 w-8"
