...
const Page = () => {
    ...
    const fetchListing = async () => {
      try {
        ...
        if (res.data?.amenities) {
          setSelected(res.data.amenities);
        }
      } catch (err) {
  ...
  // 🔹 Debounced autosave when selected changes
  useEffect(() => {
    if (!listingId) return;
  ...
  // 🔹 Final Next navigation (only updates step)
    ...
    >
      <div className="w-full min-h-svh py-20 h-full px-4">
        <div className="max-w-screen-sm w-full mx-auto pt-2">
          <h1 className="text-2xl md:text-3xl font-semibold py-4 animate-list-stagger">
                    ...
                    return (
                      <Button
                        ...
                        className={cn(
                          ...
                          isActive
                            ? "bg-white box-shadow hover:bg-white"
                            : "bg-[#F7F7F7] text-black/70"
                        )}
                        onClick={() => toggleAmenity(item.value)}
                      >
                        <div>{item.icon}</div>
                        <div className="text-left font-[550] text-sm whitespace-normal">
                          {item.label}
                        </div>
                      </Button>
                    );
