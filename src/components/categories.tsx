		...
		<div className="">
			<div className="embla 2xl:px-20 lg:px-10 md:px-8 px-0 w-full max-w-screen-2xl mx-auto relative bg-white">
					...
					<div className="embla__container gap-7 md:gap-10">
						{categoriesOptions.map((category, i) => (
							<Link href={`/${category.name.toLowerCase()}`} key={i} className="flex-grow-0 text-sm shrink-0 basis-auto max-w-[100%] min-w-0">
								<Button
									className={cn(
										"w-full flex-col px-0 font-semibold text-black transition-all rounded-none duration-200 ease-in-out hover:bg-transparent h-full py-1.5",
				...
				{canScrollNext && (
					<Button
						variant="ghost"
						className="embla__next absolute top-1/2 z-20 right-0 -translate-y-1/2 rounded-full aspect-square w-fit h-fit p-3"
						onClick={scrollNext}
						title="Next"
					>
						{Icons.rightArrow({
...
// 	return (
// 		<div className="mark">
// 			<div className="embla mark-b py-[8px] 2xl:px-20 lg:px-10 md:px-8 px-0 w-full max-w-screen-2xl mx-auto relative bg-white">
// 				<div className="embla__viewport h-full w-full" ref={emblaRef}>
// 					<div className="embla__container gap-2">
// 						{categoriesOptions.map((category, i) => (
// 							<Link href={`/${category.name.toLowerCase()}`} key={i} className="flex-grow-0 text-sm shrink-0 basis-auto max-w-[100%] min-w-0">
// 								<Button className="rounded-full w-full text-black/80 mark" size="xs" variant="outline">
// 									{category.name}
// 								</Button>
// 							</Link>
// 						))}
...
// 					<Button
// 						variant="ghost"
// 						className="embla__prev absolute top-1/2 z-20 left-0 -translate-y-1/2 rounded-full aspect-square w-fit h-fit p-2"
// 						onClick={scrollPrev}
// 						title="Prev"
// 					>
// 						{Icons.leftArrow({
